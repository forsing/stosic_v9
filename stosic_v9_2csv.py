...
"""
stosic_v9_2csv.py — 7-node krug (K=7 / prilagodjenje 7/39) — Cyclical monotonicity (7/39)
...
from stosic_v8_2csv import cooccurrence_features, cost_matrix, is_degenerate_consecutive
...
def main():
    ...
    if is_degenerate_consecutive(next_loto):
        raise SystemExit("degenerisan next_loto (uzastopni) — zaustavljen pre ispisa")
    if is_degenerate_consecutive(next_loto_plus):
        raise SystemExit("degenerisan next_loto_plus (uzastopni) — zaustavljen pre ispisa")
